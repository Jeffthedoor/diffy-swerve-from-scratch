...
public class TurdSwerve extends SubsystemBase {
	...
	public TurdSwerve() {
		...
		for (int i = 0; i < RobotMap.PodConfigs.length; i++) {
			pods.add(
					...
					new TurdPod(RobotConfig.CAN_LeftAbsoluteEncoderID, RobotConfig.leftAzimuthID, RobotConfig.leftDriveID, RobotConfig.leftOffset, RobotConfig.leftAzimuthInvert, 
					...
					RobotConfig.azimuthkI, RobotConfig.azimuthkD, RobotConfig.azimuthkS, RobotConfig.azimuthMaxOutput, RobotConfig.azimuthDriveSpeedMultiplier, RobotConfig.leftDriveInvert, 
	...
	private void initializeSuppliers() {
		DoubleSupplier globalOutputScalar = () -> pods.stream()
				.mapToDouble(TurdPod::getPodOutputScalar)
				.max()
				.orElse(0);

		BooleanSupplier isTurningSupplier = () -> pods.stream().anyMatch(TurdPod::isTurning);
