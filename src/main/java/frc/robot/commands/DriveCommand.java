...
public class DriveCommand extends Command {
	...
	public void execute() {
		...
		// orientation (+Y is robot forward)
		double speedX = MathUtil.applyDeadband(-joystickRight.get().getY(), 0.05) * maxSpeed; // Y is negated due to the
																								// way controllers work
		double speedY = MathUtil.applyDeadband(-joystickRight.get().getX(), 0.05) * maxSpeed; // X is negated due to the
																								// way field oriented
																								// works (assuming a
																								// blue origin, "right"
																								// would be negative Y)
